 ...
 *
 * Reads the flag the app config wrote at build time. A released build has no SMS
 * permission in its manifest at all, so the feature must stay hidden there
 * rather than offering a switch that could never work.
