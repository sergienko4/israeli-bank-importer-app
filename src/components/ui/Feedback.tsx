...
export function Loader({ label }: { label?: string }) {
  ...
  return (
    <View style={styles.center} accessibilityRole="progressbar" accessibilityLabel={label ?? 'Loading'}>
      ...
      {label ? (
        <Text style={[theme.typography.small, { color: theme.colors.textMuted, marginTop: 12 }]}>{label}</Text>
      ) : null}
    </View>
  );
}
...
export function ErrorView({ message, onRetry }: ErrorViewProps) {
  ...
  return (
    <View style={styles.center} accessibilityRole="alert">
      <View style={[styles.bubble, { backgroundColor: theme.colors.dangerSoft, borderRadius: theme.radius.pill }]}>
        <Ionicons name="warning-outline" size={28} color={theme.colors.danger} />
      </View>
      <Text style={[theme.typography.body, styles.message, { color: theme.colors.text }]}>{message}</Text>
      {onRetry ? (
...
const styles = StyleSheet.create({
  ...
  bubble: { width: 60, height: 60, alignItems: 'center', justifyContent: 'center', marginBottom: 12 },
